...
func (nc *Controller) handleDisruption(zoneToNodeConditions map[string][]*v1.NodeCondition, nodes []*v1.Node) {
	...
	if !allAreFullyDisrupted || !allWasFullyDisrupted {
		...
		if allAreFullyDisrupted {
			glog.Info("Controller detected that all Nodes are not-Ready. Entering master disruption mode.")
			for i := range nodes {
				if nc.useTaintBasedEvictions {
		...
		if allWasFullyDisrupted {
			glog.Info("Controller detected that some Nodes are Ready. Exiting master disruption mode.")
		...
		for k, v := range nc.zoneStates {
			...
			glog.Infof("Controller detected that zone %v is now in state %v.", k, newState)
